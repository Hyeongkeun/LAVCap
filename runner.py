...
from dist_utils import all_gather_batch
...
class Runner:
    def __init__(self, cfg, model, datasets, job_id):
        ...
        # log
        self.save_path = Path(self.config.config.run.save_path) / job_id
        self.save_path.mkdir(parents=True, exist_ok=True)

        self.model_save_freq = self.config.config.run.model_save_freq

        self.no_wandb = self.config.config.run.wandb.no_wandb
        self.wandb_entity = self.config.config.run.wandb.wandb_entity
        self.wandb_project = self.config.config.run.wandb.wandb_project
        self.wandb_name = self.config.config.run.wandb.wandb_name
        ...
        self.coco_evaluate = self.config.config.run.coco_evaluate
        self.coco_freq = self.config.config.run.coco_freq
        ...
        self.llama_tokenizer = self._model.llama_tokenizer
        ...
        self.sos_index = self.llama_tokenizer.bos_token_id
        self.eos_index = self.llama_tokenizer.eos_token_id
        ...
        logging.info('Total parameter number is : {:.3f} million'.format(sum(p.numel() for p in self.model.parameters()) / 1e6))
        logging.info('Total trainable parameter number is : {:.3f} million'.format(sum(p.numel() for p in trainables) / 1e6))
        logging.info(f'Start : base lr: {self.config.config.run.optims.init_lr}')

        # dataloaders
        self.train_sampler, self.train_loader = get_dataloader(datasets["train"], self.config.config.run, is_train=True, use_distributed=self.use_distributed)
        self.valid_sampler, self.valid_loader = get_dataloader(datasets["valid"], self.config.config.run, is_train=False, use_distributed=self.use_distributed)
        ...
        self.scheduler = LinearWarmupCosineLRScheduler(
            ...
            warmup_steps=self.config.config.run.optims.warmup_epoch * self.iters_per_epoch, # edited by lhk (LR scheduler)
    ...
    def train_epoch(self, epoch):
        ...
        for i in metric_logger.log_every(range(self.iters_per_epoch), self.config.config.run.log_freq, header=header):
            ...
            with torch.cuda.amp.autocast(enabled=self.use_amp):
                loss = self.model(samples)["loss"] / self.config.config.run.accum_grad_iters    # edited by khrho
            ...
            if (i + 1) % self.config.config.run.accum_grad_iters == 0:
                ...
                self.scheduler.step(cur_epoch=epoch, cur_step=i)    # added by khrho
            ...
            if i % self.config.config.run.log_freq:
                if is_main_process() and not self.no_wandb:
                    wandb.log({
                            "epoch": epoch,
                            "train/loss": loss.item(),
                            "train/lr": self.optimizer.param_groups[0]["lr"],
                            "train/scaler": self.scaler.get_scale() if self.use_amp else 0,
                        })
    ...
    def valid_epoch(self, epoch, split, save_json=False):
        ...
        for samples in metric_logger.log_every(dataloader, self.config.config.run.log_freq, header=header):
            ...
            metric_logger.update(loss=res["loss"])
            metric_logger.update(acc=res["acc"])
        ...
        if save_json:
            self.save_result(results, self.save_path, "eval_epoch_{}".format(epoch))
    ...
    @torch.no_grad()
    def COCO_epoch(self, epoch, split, save_json=False):
        header = "COCO - Eval: data epoch: [{}]".format(epoch)
        ...
        for samples in metric_logger.log_every(dataloader, self.config.config.run.log_freq, header=header):
            ...
            if model.prompt_dict:
                ...
                if self.test_prompt_dict is None:
                    ...
                    for i in range(len(samples['torch_id'])):
                        prompts.append(self.test_prompt_dict["audiocaption"])
            else:
                prompts = None
                
            with torch.cuda.amp.autocast(enabled=self.use_amp):
                forward_result = model.generate(samples, self.config.config.generate, prompts=prompts)

            A_tid.append(forward_result['torch_id'])
            A_pred.append(forward_result['predictions'])
        ...
        all_id, all_pred = all_gather_batch([tid_output, pred_output])
        ...
        for tid, predict in zip(all_id, all_pred):
            ...
            if eos_position is not None:
                result = predict[:eos_position]
            else:
                result = []
                
            coco_json['image_id'] = self.torch2iid[str(tid.item())]
            coco_json['caption'] = self.llama_tokenizer.decode(result)
            coco_result_json.append(coco_json)

        if save_json:
            self.save_result(coco_json, self.save_path, "COCO_eval_{}_epoch_{}".format(split, epoch))

        return coco_result_json
    ...
    def train(self):
        ...
        if is_main_process() and not self.no_wandb:
            wandb.init(project=self.wandb_project, entity=self.wandb_entity, config=self, resume='allow', name=self.wandb_name)
        ...
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            ...
            if self.use_distributed:
                ...
                self.valid_sampler.set_epoch(cur_epoch)
            ...
            valid_log = self.valid_epoch(cur_epoch, "valid", save_json=False)
            if valid_log is not None:
                if is_main_process():
                    ...
                    if not self.no_wandb:
                        wandb.log({f'eval/{k}': v for k, v in valid_log.items()})

            logging.info(f"Validating at {cur_epoch} Finish")

            self.save_checkpoint(cur_epoch, is_best=False)

            if cur_epoch % self.coco_freq == 0 and self.coco_evaluate:
                # validating phase
                logging.info("COCO Validating Phase")
                coco_result_json = self.COCO_epoch(cur_epoch, "valid", save_json=True)
                json_save_path = os.path.join(self.save_path,f"epoch{str(cur_epoch)}_coco.json")

                if is_main_process():
                    with open(json_save_path, 'w') as f:
                        json.dump(coco_result_json, f)

                    annotation_file = self.config.config.datasets.annot_path
                    ...
                    if not self.no_wandb:
                        wandb.log({f'coco_{k}': v for k, v in stats.items()})
                
                logging.info(f"COCO Validating at {cur_epoch} Finish")
        ...
        # testing phase
        if self.evaluate_only:
            coco_result_json = self.COCO_epoch("Best", "valid", save_json=True)
            json_save_path = os.path.join(self.save_path,f"test_only_coco.json")
